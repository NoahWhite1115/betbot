...
from exceptions.ddbExceptions import playerNotFoundException, \
    channelNotFoundException
from helpers.conversion_helper import responseToPlayerData, \
    responseToBetData, responseToAllPlayerData, responseToAllBetData
        ...
        try:
            response = self.table.query(
                KeyConditionExpression=(
                    Key('game_id').eq(str(channelId)) &
                    Key('user_id').eq('bet_data')),
    ...
    def getPlayerData(self, channelId, playerId):
        try:
            response = self.table.query(
                KeyConditionExpression=(
                    Key('game_id').eq(str(channelId)) &
                    Key('user_id').eq(str(playerId))),
                ScanIndexForward=False
            )
        except self.dynamodb.meta.client.exceptions.ResourceNotFoundException:
            raise(playerNotFoundException('No channel with id '
                                          + str(channelId)))
